...
import { createTypeEnvironment, isBuiltInTypeReference, type TypeEnvironment } from "../shared/dictionary-types.ts";
...
function isBroadRecordKeyType(type: ESTree.TSType, environment: TypeEnvironment): boolean {
  ...
  return unwrapped.type === "TSTypeReference" &&
    isBuiltInTypeReference(unwrapped, "PropertyKey", environment);
}
...
function isBroadRecordType(type: ESTree.TSType, environment: TypeEnvironment): boolean {
  ...
  if (unwrapped.type === "TSTypeReference") {
    if (isBuiltInTypeReference(unwrapped, "Readonly", environment)) {
      const [inner] = unwrapped.typeArguments?.params ?? [];
      return inner !== undefined && isBroadRecordType(inner, environment);
    }

    if (!isBuiltInTypeReference(unwrapped, "Record", environment)) return false;
...
function isDefinitelyNarrowerRecordType(type: ESTree.TSType, environment: TypeEnvironment): boolean {
  const unwrapped = unwrapTypeParentheses(type);
...
function widenedBinding(
  ...
  const initializerBroadKind =
    initializerAssertion === null ? null : broadTypeKind(initializerAssertion.typeAnnotation, environment);
  const declaredBroadKind = declaredType === undefined ? null : broadTypeKind(declaredType, environment);
